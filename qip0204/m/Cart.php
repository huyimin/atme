    ...
    public function createOrderAndClearCart($userId) {
        ...
        try {
            ...
            foreach ($cartItems as $item) {
                ...
                $this->db->query("INSERT INTO orders (order_number, user_id, item_id, item_name, unit_price, quantity, stock) 
                                  VALUES ('$orderNumber', '$userId', '{$item['item_id']}', '{$item['item_name']}', '{$item['price']}', '{$item['quantity']}', '{$item['quantity']}')
                                  ON DUPLICATE KEY UPDATE 
                                  quantity = quantity + VALUES(quantity), 
                                  stock = stock + VALUES(stock)");
                if ($this->db->countAffected() <= 0) {
                    error_log('Failed to insert or update order for user: ' . $userId);
                    throw new Exception('Failed to create or update order.');
                }
            }
            ...
            $this->db->query("COMMIT");
            return ['success' => true, 'message' => 'Order created or updated and cart cleared'];
        } catch (Exception $e) {
            $this->db->query("ROLLBACK");
