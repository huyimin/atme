    ...
    if (isset($data['action'])) {
        switch ($data['action']) {
            ...
            case 'return':
                ...
                break;
            case 'finalize_deal':
                $result = $orderController->finalizeDeal($data['itemIds']);
                echo json_encode($result);
                break;
            // case 'confirm_invoice':
            //     $result = $orderController->confirmInvoice($_SESSION['user_id']);
            //     echo json_encode($result);
            //     break; 
            // case 'confirm_invoice':
            //     if (!$orderModel->isUserAdmin($_SESSION['user_id'])) {
            //         echo json_encode(['success' => false, 'message' => 'Unauthorized']);
            //         exit;
            //     }
            //     $invoiceId = $data['invoiceId']; // The ID of the invoice to confirm
            //     $result = $orderController->confirmInvoice($invoiceId);
            //     echo json_encode($result);
            //     break;  
            case 'confirm_invoice':
                if (!$orderModel->isUserAdmin($_SESSION['user_id'])) {
                ...
                // Ensure that 'invoiceId' is passed and is a number
                if (!isset($data['invoiceId']) || !is_numeric($data['invoiceId'])) {
                    echo json_encode(['success' => false, 'message' => 'Invoice ID is required and must be a number.']);
                    exit;
                }
                $invoiceId = $data['invoiceId']; // The ID of the invoice to confirm
                $result = $orderController->confirmInvoice($invoiceId);
